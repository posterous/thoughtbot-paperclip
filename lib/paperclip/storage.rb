module Paperclip
  module Storage
    ...
    module S3
      ...
      def flush_writes #:nodoc:
        logger.info("[paperclip1] Writing files for #{name}")
        @queued_for_write.each do |style, file|
          begin
            logger.info("[paperclip1] -> bucket #{s3_bucket} path #{path(style)}")
      ...
      def flush_deletes #:nodoc:
        logger.info("[paperclip2] Deleting files for #{name}")
        @queued_for_delete.each do |path|
          begin
            logger.info("[paperclip2] -> bucket #{s3_bucket} path #{path}")
